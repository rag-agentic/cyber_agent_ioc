...
class ProcessLogToolConfig(FunctionBaseConfig, name="process_log_tool"):
    """
...
async def process_log_tool(config:ProcessLogToolConfig, builder: Builder):
    ...
    async def _arun(duration: int) -> str:
        ...
        try:
            ...
            user_prompt = ThreatProcessLogPrompts.USER_INSTRUCTIONS_PROMPT.format(input_data=output_for_prompt)

            prompt = ChatPromptTemplate([SystemMessage(content=ThreatProcessLogPrompts.SYSTEM_INSTRUCTIONS_PROMPT),
            ...
            logger.error(f"Error in process log system agent: {str(e)}")
            return f"Sorry, I encountered an error while generating your analyst process monitor: {str(e)}"
